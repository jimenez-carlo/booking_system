<div class="row">
  <div class="col-lg-12">
    <h1 class="page-header"><i class="fa fa-user-edit"></i> Editing Account</h1>
  </div>
</div>
<div class="row">
  <form action="" method="post" name="edit_account" refresh="admin_account_edit&id=<?= $info->id ?>">
              ...
              <div class="form-group">
                ...
                <select class="form-control select2" name="account_type" id="account_type">
                  ...
                  <?php foreach ($account_type as $res) { ?>
                    ...
                    <option value="<?= $res['id'] ?>" data-child="<?= $res['child'] ?>" <?= $res['id'] == $info->account_type_id ? 'selected' : '' ?>><?= $res['name'] ?></option>
          ...
          <div class="row" id="subaccount">
            <div class="col-lg-3">
              <div class="form-group">
                ...
                <input type="checkbox" aria-label="..." class="" name="is_sub_account" id="is_sub_account" <?= ($info->is_sub_account) ? "checked" : "" ?>>
              </div>
            </div>
...
<script>
  $(document).ready(function() {
    ...
    // $('.select2').select2();
    parent_account(<?= !empty($info->parent_account_id) ? $info->parent_account_id : 0 ?>);
    toggleSubAccountSection();
    toggleSubAccountCheckbox();
  });
</script>