...
if (in_array($page, $pages)) {
  ...
  switch ($page) {
    ...
    case 'admin_customer_edit':
      ...
      $data['invoice_history'] = $request->get_list("SELECT  t.due_date,sum(if(ti.reference_type ='profit', IFNULL(ti.amount,0), 0)) - (sum(if(ti.reference_type ='expense', IFNULL(ti.amount,0), 0)) + sum(IFNULL(tx.amount,0)))   as invoice_total,  sum(IFNULL(p.amount,0)) as payment_total,ti.*,c.name as company FROM tbl_invoice t left join tbl_invoice_transaction ti on ti.invoice = t.invoice left join tbl_company c on c.id = ti.company_id left join tbl_invoice_payment p on p.invoice = t.invoice left join tbl_invoice_tax tx on tx.invoice = t.invoice where ti.reference_type <> 'disbursement' and ti.customer_id = $id and t.is_deleted = 0 group by t.invoice,ti.company_id");
      break;
